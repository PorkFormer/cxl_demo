#!/usr/bin/env python3
"""Run the Spark MLlib RDD Logistic Regression workload on HIGGS."""
...
DEFAULT_OUTPUT = Path("logs/spark_mllib_higgs_logreg_metrics.json")


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train only Spark MLlib RDD Logistic Regression on HIGGS."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    metrics = run_workload(
        args,
        algorithms=[AlgorithmSpec(ALGORITHM_NAME, "LBFGS", args.iterations)],
